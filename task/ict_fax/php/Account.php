...
class Account
{
  ...
  public static function search($aFilter = array())
  {
    ...
    $query = "SELECT a.account_id, a.tenant_id, a.type, a.username, a.first_name, a.last_name, a.phone, a.email,  a.company, a.address,a.created_by
              FROM account a LEFT JOIN tenant t ON t.tenant_id = a.tenant_id" . $from_str;
  ...
  public function associate($user_id, $aUser = array())
  {
    ...
    if ($user_id != 'NULL') {
      ...
      if (!empty($aUser)) {
        ...
        $this->phone = $aUser[0]['phone'];
        $this->email = null;
      }
    } else {
      $this->tenant_id = $user_id;
    }
    $query = "UPDATE " . self::$table . " SET tenant_id=%tenant_id%, created_by=%user_id%, first_name='%first_name%', last_name='%last_name%',
     phone='%phone%', email=NULL WHERE account_id=%account_id%";
    $result = DB::query(self::$table, $query, array('tenant_id' => $this->tenant_id, 'first_name' => $this->first_name,'last_name' => $this->last_name,
     'phone' => $this->phone, 'email' => $this->email,   'user_id' => $user_id,'account_id' => $this->account_id));   
